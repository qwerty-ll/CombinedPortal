...
def main():
    ...
    for i, (q, ans) in enumerate([
        ...
        ("Где находится коворкинг ИВИТШ?", "На 4 этаже корпуса Б. Открыт с 9:00 до 20:00, вход по студенческому."),
        ("Как перевестись в другую группу?", "Напишите заявление в дирекции ИВИТШ (Б-209) до конца первой недели семестра."),
        ("Что делать, если заболел?", "Сообщите куратору и принесите справку в дирекцию в течение трёх дней после выздоровления."),
        ("Где взять учебники?", "В библиотеке корпуса А по студенческому билету. Электронные версии есть в ЭИОС."),
        ("Как получить место в общежитии?", "Подайте заявление в профком с копией паспорта; очередь формируется в августе."),
    ], 1):
    ...
    print("Готово: 3 объявления, 8 вопросов FAQ, 3 вопроса на форуме, прогресс студента 25-isbo-011.")
