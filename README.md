...
- 📝 **Документы от ВИТШика**: попросите кота «напиши объяснительную, вчера болел» или «заявление на пересдачу по философии». В чате появится готовая объяснительная записка или заявление: ФИО в нужном падеже, группа и курс, пропущенные пары и преподаватель берутся из профиля и расписания ЭИОС. Поля можно поправить прямо в карточке. Документ скачивается в Word (Times New Roman 14, чтобы при необходимости поправить) или в PDF (для печати как есть). Кому адресованы документы, задаётся в `DOCUMENT_ADDRESSEE_TITLE` / `DOCUMENT_ADDRESSEE_NAME` (в дательном падеже), без имени остаётся строка для подписи от руки.
