...
- 🤖 **ВИТШик — помощник с данными портала**: отвечает, где следующая пара, что завтра, когда предмет, как найти аудиторию (со схемой этажа) и где сейчас преподаватель. Ищет ответы в частых вопросах, на форуме и в базе знаний и добавляет к ответу кнопки: «на карте», «обсуждение», «расписание». Время и аудитории считаются по расписанию ЭИОС и не проходят через языковую модель. GigaChat только перефразирует найденные тексты, и только для вошедших пользователей.
- 👨‍🏫 **Справочник преподавателей**: кафедры, кабинеты, степени и фотографии **17 преподавателей ИВИТШ КГУ**. По расписанию ЭИОС видно, где преподаватель сейчас («На паре в Б-305 до 11:40», «Свободна до 13:40»), а в карточке — его пары на сегодня.
- 🗓️ **Расписание (интеграция с ЭИОС КГУ)**: данные ЭИОС кэшируются на сервере; если ЭИОС недоступна, показывается последняя сохранённая копия с пометкой либо честное сообщение об ошибке — без выдуманных занятий. Расписание группы можно подписать в календарь телефона (Apple, Google и другие, `GET /api/v1/calendar/group.ics?name=…`), и оно обновляется само.
- 📱 **Приложение на телефоне (PWA)**: портал устанавливается на главный экран и открывается без интернета: service worker хранит само приложение, а API-клиент — последние расписание, объявления и преподавателей. Ответы API с личными данными не кэшируются.
